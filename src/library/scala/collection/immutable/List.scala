...
sealed abstract class List[+A] extends AbstractSeq[A]
  ...
  @inline final def mapConserve[B >: A <: AnyRef](f: A => B): List[B] = {
    ...
    var unchanged: List[A] = this
    var pending: List[A] = this
    var mappedHead: List[B] = null
    var mappedLast: ::[B] = null

    while (pending ne Nil) {
      val head0 = pending.head
      val head1 = f(head0)
      if (head1.asInstanceOf[AnyRef] eq head0.asInstanceOf[AnyRef]) pending = pending.tail
      else {
        var xc = unchanged
        while (xc ne pending) {
          val next = new ::[B](xc.head, Nil)
          if (mappedHead eq null) mappedHead = next
          else mappedLast.tl = next
          mappedLast = next
          xc = xc.tail
        }
        val next = new ::[B](head1, Nil)
        if (mappedHead eq null) mappedHead = next
        else mappedLast.tl = next
        mappedLast = next
        unchanged = pending.tail
        pending = unchanged
      }
    }
